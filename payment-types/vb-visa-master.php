...
add_filter( 'woocommerce_payment_gateways', 'vb_visa_master_add_gateway_class' );
function vb_visa_master_add_gateway_class( $gateways ) {
	$gateways[] = 'WC_Victoriabank_Visa_MasterCard_payment';
	return $gateways;
}

add_action( 'plugins_loaded', 'vb_visa_master_init_gateway_class');
function vb_visa_master_init_gateway_class() {

	class WC_Victoriabank_Visa_MasterCard_payment extends WC_Payment_Gateway {

 		public function __construct() {
            $this->id = 'vb_visa_mastercard';
            $this->icon = apply_filters( 'woocommerce_gateway_icon', plugin_dir_url('').'vb-payment-plugin\assets\images\vb_240_x_240_px.png' );
            $this->method_title = 'Victoriabank Visa/MasterCard';
            $this->method_description = __('WooCommerce Payment Gateway for Victoriabank Visa/MasterCard', 'victoriabank-payment-gateway');
	        ...
	        $this->title = 'Visa/MasterCard/Integral Star Card';
		...
		public function payment_fields() {
            ...
            echo '<div class="payment-description">' . $options['description'] . '</div>';
		}
        ...
        public function get_icon() {
            $logo_file = plugin_dir_url(''). 'vb-payment-plugin/' . get_logo_visa('Logo for Visa/Mastercard');

            $existed_logo = file_exists($logo_file) ? '<img src="' . $logo_file . '" alt="Payment Icon">' : '<img src="' . $this->icon . '" alt="Payment Icon">';
            $enabled = get_logo_visa('Enable logo for Visa/Mastercard') === 'on';
...
function get_logo_visa($field_name) {
    global $wpdb;
